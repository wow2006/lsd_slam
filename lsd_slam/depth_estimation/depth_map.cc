...
void DepthMap::observeDepth() {
  ...
  if (enablePrintDebugInfo && printObserveStatistics) {
    printf("OBSERVE (%d): %d / %d created; %d / %d updated; %d skipped; %d "
           "init-blacklisted\n",
  ...
  if (enablePrintDebugInfo && printObservePurgeStatistics) {
    printf("OBS-PRG (%d): Good: %d; inconsistent: %d; notfound: %d; oob: %d; "
           "failed: %d; addSkip: %d;\n",
...
void DepthMap::propagateDepth(Frame *new_keyframe) {
  ...
  if (new_keyframe->getTrackingParent() != activeKeyFrame) {
    printf("WARNING: propagating depth from frame %d to %d, which was tracked "
           "on a different frame (%d).\nWhile this should work, it is not "
           "recommended.",
  ...
  if (enablePrintDebugInfo && printPropagationStatistics) {
    printf("PROPAGATE: %d: %d drop (%d oob, %d color); %d created; %d merged; "
           "%d occluded. %d col-dec, %d grad-dec.\n",
           runningStats.num_prop_attempts,
...
void DepthMap::regularizeDepthMap(bool removeOcclusions, int validityTH) {
  ...
  if (enablePrintDebugInfo && printRegularizeStatistics)
    printf("REGULARIZE (%d): %d smeared; %d blacklisted /%d new); %d deleted; "
           "%d occluded; %d filled\n",
...
void DepthMap::updateKeyframe(
    std::deque<std::shared_ptr<Frame>> referenceFrames) {
  assert(isValid());
  ...
  for (std::shared_ptr<Frame> frame : referenceFrames) {
    ...
    if (frame->getTrackingParent() != activeKeyFrame) {
      printf("WARNING: updating frame %d with %d, which was tracked on a "
             "different frame (%d).\nWhile this should work, it is not "
             "recommended.",
  ...
  if (enablePrintDebugInfo && printLineStereoStatistics) {
    printf("ST: calls %6d, comp %6d, int %7d; good %6d (%.0f%%), neg %6d "
           "(%.0f%%); interp %6d / %6d / %6d\n",
  ...
  if (enablePrintDebugInfo && printLineStereoFails) {
    printf("ST-ERR: oob %d (scale %d, inf %d, near %d); err %d (%d uncl; %d "
...
void DepthMap::addTimingSample() {
  ...
  if (sPassed > 1.0f) {
    ...
    if (enablePrintDebugInfo && printMappingTiming) {
      printf("Upd %3.1fms (%.1fHz); Create %3.1fms (%.1fHz); Final %3.1fms "
             "(%.1fHz) // Obs %3.1fms (%.1fHz); Reg %3.1fms (%.1fHz); Prop "
             "%3.1fms (%.1fHz); Fill %3.1fms (%.1fHz); Set %3.1fms (%.1fHz)\n",
             msUpdate, nAvgUpdate, msCreate, nAvgCreate, msFinalize,
             nAvgFinalize, msObserve, nAvgObserve, msRegularize, nAvgRegularize,
             msPropagate, nAvgPropagate, msFillHoles, nAvgFillHoles, msSetDepth,
             nAvgSetDepth);
    }
  }
}
...
inline float DepthMap::doLineStereo(
  ...
  if (pClose[0] <= SAMPLE_POINT_TO_BORDER ||
    ...
    if (pClose[0] <= SAMPLE_POINT_TO_BORDER ||
        pClose[0] >= width - SAMPLE_POINT_TO_BORDER ||
        pClose[1] <= SAMPLE_POINT_TO_BORDER ||
        pClose[1] >= height - SAMPLE_POINT_TO_BORDER || newEplLength < 8) {
      if (enablePrintDebugInfo)
