...
std::unordered_set<Frame *>
TrackableKeyFrameSearch::findCandidates(Frame *keyframe,
  ...
  if (enablePrintDebugInfo && printConstraintSearchInfo)
    printf("Early LoopClosure-Candidates for %d: %d euclidean, %d "
...
#else
  if (useFabMap)
    printf("Warning: Compiled without FabMap, but useFabMap is enabled... "
           "ignoring.\n");
  return nullptr;
#endif
}
