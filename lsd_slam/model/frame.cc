...
void Frame::prepareForStereoWith(Frame *other, const Sim3 &thisToOther,
  ...
  K_otherToThis_R =
      K * otherToThis.rotationMatrix().cast<float>() * otherToThis.scale();
...
void Frame::buildImage(int level) {
  if (level == 0) {
    printf("Frame::buildImage(0): Loading image from disk is not implemented "
           "yet! No-op.\n");
    return;
  }
...
void Frame::releaseImage(int level) {
  if (level == 0) {
    printf("Frame::releaseImage(0): Storing image on disk is not supported "
           "yet! No-op.\n");
    return;
  }
  FrameMemory::getInstance().returnBuffer(data.image[level]);
...
void Frame::buildIDepthAndIDepthVar(int level) {
  ...
  if (level == 0) {
    printf("Frame::buildIDepthAndIDepthVar(0): Loading depth from disk is not "
...
void Frame::releaseIDepth(int level) {
  if (level == 0) {
    printf("Frame::releaseIDepth(0): Storing depth on disk is not supported "
           "yet! No-op.\n");
    return;
  }
...
void Frame::releaseIDepthVar(int level) {
  if (level == 0) {
    printf("Frame::releaseIDepthVar(0): Storing depth variance on disk is not "
           "supported yet! No-op.\n");
    return;
  }
  FrameMemory::getInstance().returnBuffer(data.idepthVar[level]);
