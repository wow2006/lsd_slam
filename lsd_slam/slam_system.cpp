...
SlamSystem::SlamSystem(int w, int h, Eigen::Matrix3f K, bool enableSLAM)
  ...
  if (w % 16 != 0 || h % 16 != 0) {
    printf("image dimensions must be multiples of 16! Please crop your images "
           "/ video accordingly.\n");
    assert(false);
  }
  ...
  m_pMap = new DepthMap(w, h, K);

  newConstraintAdded = false;
  haveUnmergedOptimizationOffset = false;

  m_pTracker = new SE3Tracker(w, h, K);
  // Do not use more than 4 levels for odometry tracking
  for (int level = 4; level < PYRAMID_LEVELS; ++level)
    m_pTracker->settings.maxItsPerLvl[level] = 0;
  ...
  while (keepRunning) {
    if (newKeyFrames.size() == 0) {
      ...
      msFindConstraintsItaration =
          ...
          0.1 *
              std::chrono::duration_cast<std::chrono::milliseconds>(tv_end -
                                                                    tv_start)
                  .count();
...
void SlamSystem::discardCurrentKeyframe() {
  ...
  if (currentKeyFrame->idxInKeyframes >= 0) {
    printf("WARNING: trying to discard a KF that has already been added to "
           "the graph... finalizing instead.\n");
...
void SlamSystem::createNewCurrentKeyframe(std::shared_ptr<Frame> newKeyframeCandidate) {
  ...
  // propagate & make new.
  m_pMap->createKeyFrame(newKeyframeCandidate.get());
...
void SlamSystem::loadNewCurrentKeyframe(Frame *keyframeToLoad) {
  ...
  m_pMap->setFromExistingKF(keyframeToLoad);
  ...
  if (newReferenceKF != 0)
    ...
    if (force) {
      if (noCreate) {
        ...
        nextRelocIdx = -1;
        printf("mapping is disabled & moved outside of known map. "
...
void SlamSystem::addTimingSamples() {
  m_pMap->addTimingSample();
  // std::chrono::high_resolution_clock::time_point now;
  // gettimeofday(&now, NULL);
  auto now = std::chrono::high_resolution_clock::now();
  float sPassed =
      std::chrono::duration_cast<std::chrono::seconds>(now - lastHzUpdate)
          .count();
  if (sPassed > 1.0f) {
    ...
    if (enablePrintDebugInfo && printOverallTiming) {
      printf("MapIt: %3.1fms (%.1fHz); Track: %3.1fms (%.1fHz); Create: "
             ...
             "(%.1fHz);\n",
             m_pMap->msUpdate, m_pMap->nAvgUpdate, msTrackFrame, nAvgTrackFrame,
             m_pMap->msCreate + m_pMap->msFinalize, m_pMap->nAvgCreate, msFindReferences,
             nAvgFindReferences,
             trackableKeyFrameSearch != 0
...
void SlamSystem::debugDisplayDepthMap() {
  ...
  snprintf(buf2, 200,
           "dens %2.0f%%; good %2.0f%%; scale %2.2f; res %2.1f/; usg "
           "%2.0f%%; Map: %d F, %d KF, %d E, %.1fm Pts",
  ...
  if (onSceenInfoDisplay) {
    printMessageOnCVImage(m_pMap->debugImageDepth, buf1, buf2);
  }
  if (displayDepthMap) {
    Util::displayImage("DebugWindow DEPTH 1", m_pMap->debugImageDepth, false);
  }
...
void SlamSystem::takeRelocalizeResult() {
  ...
  m_pTracker->trackFrame(trackingReference, succFrame.get(), succFrameToKF_init);

  if (!m_pTracker->trackingWasGood ||
...
bool SlamSystem::doMappingIteration() {
  ...
  if (trackingIsGood) {
    ...
    if (createNewKeyFrame) {
      ...
      const bool didSomething = updateKeyframe();
...
void SlamSystem::gtDepthInit(uchar *image, float *depth, double timeStamp, int id) {
  ...
  m_pMap->initializeFromGTDepth(currentKeyFrame.get());
...
void SlamSystem::randomInit(uchar *image, double timeStamp, int id) {
  std::cout << "Doing Random initialization!\n";

  if(!doMapping) {
    std::cerr << "WARNING: mapping is disabled, but we just initialized... THIS WILL NOT WORK! Set doMapping to true.\n";
  }
  ...
  currentKeyFrame.reset(new Frame(id, width, height, K, timeStamp, image));
  //
  m_pMap->initializeRandomly(currentKeyFrame.get());
  keyFrameGraph->addFrame(currentKeyFrame.get());
  ...
  std::cout << "Done Random initialization!\n";
}

void SlamSystem::trackFrame(uchar *image, unsigned int frameID, bool blockUntilMapped, double timestamp) {
  // Create new frame
  auto trackingNewFrame = std::make_shared<Frame>(frameID, width, height, K, timestamp, image);
  ...
  const bool my_createNewKeyframe = createNewKeyFrame;
  ...
  SE3 newRefToFrame_poseUpdate = m_pTracker->trackFrame(trackingReference, trackingNewFrame.get(), frameToReference_initialEstimate);
  ...
  tracking_lastGoodPerBad   = m_pTracker->lastGoodCount / (m_pTracker->lastGoodCount + m_pTracker->lastBadCount);
  tracking_lastGoodPerTotal = m_pTracker->lastGoodCount / (trackingNewFrame->width(SE3TRACKING_MIN_LEVEL)*trackingNewFrame->height(SE3TRACKING_MIN_LEVEL));

  if (manualTrackingLossIndicated || m_pTracker->diverged || (keyFrameGraph->keyframesAll.size() > INITIALIZATION_PHASE_COUNT && !m_pTracker->trackingWasGood)) {
  ...
  if (plotTracking) {
    Eigen::Matrix<float, 20, 1> data;
    data.setZero();
    data[0] = m_pTracker->lastResidual;

    data[3] = m_pTracker->lastGoodCount /
              (m_pTracker->lastGoodCount + m_pTracker->lastBadCount);
    data[4] = 4 * m_pTracker->lastGoodCount / (width * height);
    data[5] = m_pTracker->pointUsage;
  ...
  if (!my_createNewKeyframe && currentKeyFrame->numMappedOnThisTotal > MIN_NUM_MAPPED) {
    ...
    lastTrackingClosenessScore = trackableKeyFrameSearch->getRefFrameScore(dist.dot(dist), m_pTracker->pointUsage);
...
int SlamSystem::findConstraintsForNewKeyFrames(Frame *newKeyFrame,
  ...
  for (std::unordered_set<Frame *>::iterator c = candidates.begin();
    ...
    if (newKeyFrame->neighbors.find(*c) != newKeyFrame->neighbors.end()) {
      if (enablePrintDebugInfo && printConstraintSearchInfo)
        printf("SKIPPING %d on %d cause it already exists as "
               "constraint.\n",
               (*c)->id(), newKeyFrame->id());
  ...
  for (std::unordered_set<Frame *>::iterator c = closeCandidates.begin();
    ...
    if (skip) {
      if (enablePrintDebugInfo && printConstraintSearchInfo)
        printf("SKIPPING %d on %d (NEAR), cause we already have tried "
               "it.\n",
               (*c)->id(), newKeyFrame->id());
  ...
  for (unsigned int i = 0; i < farCandidates.size(); i++) {
    ...
    if (skip) {
      if (enablePrintDebugInfo && printConstraintSearchInfo)
        printf("SKIPPING %d on %d (FAR), cause we already have tried "
               "it.\n",
  ...
  if (enablePrintDebugInfo && printConstraintSearchInfo)
    printf("Final Loop-Closure Candidates: %d / %d close (%d failed, %d "
           "inconsistent) + %d / %d far (%d failed, %d inconsistent) = %d\n",
           (int)closeCandidates.size(), closeAll, closeFailed,
           closeInconsistent, (int)farCandidates.size(), farAll, farFailed,
           farInconsistent,
  ...
  if (parent != 0 && forceParent) {
    ...
    if (e1 != 0) {
      ...
      printf("warning: reciprocal tracking on new frame failed badly, added "
             "odometry edge (Hacky).\n");
...
bool SlamSystem::optimizationIteration(int itsPerTry, float minChange) {
  ...
  if (enablePrintDebugInfo && printOptimizationInfo)
    printf("did %d optimization iterations. Max Pose Parameter Change: %f; "
           "avgChange: %f. %s\n",
