...
inline void NormalEquationsLeastSquares::update(const Vector6 &J,
  ...
  // MathSse<Sse::Enabled, float>::add(b, J, -res * factor); // not much
  // difference :(
  b -= J * (res * weight);
...
void NormalEquationsLeastSquares::combine(
    const NormalEquationsLeastSquares &other) {
  A_opt += other.A_opt;
  b += other.b;
  error += other.error;
...
void NormalEquationsLeastSquares::finishNoDivide() { A_opt.toEigen(A); }
...
// TODO: Ugly temporary replacement for SSE instructions to make rankUpdate()
// work.
...
__m128 _mm_set1_ps(float v) { return __m128(v, v, v, v); }
__m128 _mm_loadu_ps(const float *d) { return __m128(d[0], d[1], d[2], d[3]); }
__m128 _mm_load_ps(const float *d) { return __m128(d[0], d[1], d[2], d[3]); }
...
inline void
OptimizedSelfAdjointMatrix6x6f::rankUpdate(const Eigen::Matrix<float, 6, 1> &u,
                                           const float alpha) {
  ...
  __asm__ __volatile__(
      // NOTE: may reduce count of used registers by calculating some value(s?)
      // later.
      ...
      : /* outputs */
      : /* inputs  */ [alpha] "r"(alpha), [in_ptr] "r"(in_ptr),
        [out_ptr] "r"(out_ptr)
      : /* clobber */ "memory", "cc", // TODO: is cc necessary?
        "q0", "q1", "q2", "q3", "q4", "q5", "q8", "q9", "q10", "q11", "q12",
        "q13", "q14");
  ...
  _mm_store_ps(data + 0,
               _mm_add_ps(_mm_load_ps(data + 0), _mm_mul_ps(v1122, v1212)));
  _mm_store_ps(data + 4,
               _mm_add_ps(_mm_load_ps(data + 4), _mm_mul_ps(v1122, v3434)));
  _mm_store_ps(data + 8,
               _mm_add_ps(_mm_load_ps(data + 8), _mm_mul_ps(v1122, v5656)));

  __m128 v3344 = _mm_mul_ps(s, _mm_unpacklo_ps(v3434, v3434));

  _mm_store_ps(data + 12,
               _mm_add_ps(_mm_load_ps(data + 12), _mm_mul_ps(v3344, v3434)));
  _mm_store_ps(data + 16,
               _mm_add_ps(_mm_load_ps(data + 16), _mm_mul_ps(v3344, v5656)));

  __m128 v5566 = _mm_mul_ps(s, _mm_unpacklo_ps(v5656, v5656));

  _mm_store_ps(data + 20,
               _mm_add_ps(_mm_load_ps(data + 20), _mm_mul_ps(v5566, v5656)));

#endif
}

inline void OptimizedSelfAdjointMatrix6x6f::

operator+=(const OptimizedSelfAdjointMatrix6x6f &other) {
#if defined(ENABLE_SSE)
  _mm_store_ps(data + 0,
               _mm_add_ps(_mm_load_ps(data + 0), _mm_load_ps(other.data + 0)));
  _mm_store_ps(data + 4,
               _mm_add_ps(_mm_load_ps(data + 4), _mm_load_ps(other.data + 4)));
  _mm_store_ps(data + 8,
               _mm_add_ps(_mm_load_ps(data + 8), _mm_load_ps(other.data + 8)));
  ...
  __asm__ __volatile__(
      ...
      : /* inputs  */ [data] "r"(data)
      : /* clobber */ "memory", "q0", "q1", "q2", "q3", "q4", "q5", "q9", "q10",
        "q11", "q12", "q13", "q14");
#else
  for (size_t idx = 0; idx < Size; idx++)
...
void OptimizedSelfAdjointMatrix6x6f::toEigen(
    Eigen::Matrix<float, 6, 6> &m) const {
...
void NormalEquationsLeastSquares4::combine(
    const NormalEquationsLeastSquares4 &other) {
  A += other.A;
  b += other.b;
  error += other.error;
  num_constraints += other.num_constraints;
...
void NormalEquationsLeastSquares7::initializeFrom(
    const NormalEquationsLeastSquares &ls6,
...
void NormalEquationsLeastSquares7::combine(
    const NormalEquationsLeastSquares7 &other) {
  A += other.A;
  b += other.b;
  error += other.error;
  num_constraints += other.num_constraints;
