      ...
      while (true) {
        ...
        if (error.mean < lastErr.mean) {
          ...
          if (enablePrintDebugInfo && printTrackingIterationInfo) {
            // debug output
            printf("(%d-%d): ACCEPTED increment of %f with lambda %.1f, "
                   "residual: %f -> %f\n",
          ...
          if (error.mean / lastErr.mean > settings.convergenceEps[lvl]) {
            if (enablePrintDebugInfo && printTrackingIterationInfo) {
              printf("(%d-%d): FINISHED pyramid level (last residual reduction "
                     "too small).\n",
                     lvl, iteration);
            }
          ...
          if (enablePrintDebugInfo && printTrackingIterationInfo) {
            printf("(%d-%d): REJECTED increment of %f with lambda %.1f, "
                   "(residual: %f -> %f)\n",
                   lvl, iteration, sqrt(inc.dot(inc)), LM_lambda, lastErr.mean,
                   error.mean);
          }
...
#if defined(ENABLE_SSE)
void Sim3Tracker::calcSim3BuffersSSE(const TrackingReference *reference,
                                     Frame *frame, const Sim3 &referenceToFrame,
...
#if defined(ENABLE_NEON)
void Sim3Tracker::calcSim3BuffersNEON(const TrackingReference *reference,
                                      Frame *frame,
                                      const Sim3 &referenceToFrame, int level,
                                      bool plotWeights) {
  calcSim3Buffers(reference, frame, referenceToFrame, level, plotWeights);
}
#endif

void Sim3Tracker::calcSim3Buffers(const TrackingReference *reference,
                                  Frame *frame, const Sim3 &referenceToFrame,
                                  int level, bool plotWeights) {
  if (plotSim3TrackingIterationInfo) {
...
// save values
    ...
    *(buf_d + idx) = 1.0f / (*refPoint)[2];
    ...
    if (plotSim3TrackingIterationInfo) {
      ...
      setPixelInCvMat(&debugImageResiduals, getGrayCvPixel(residual_p + 128), x,
                      y, (width / w));
...
Sim3Tracker::calcSim3WeightsAndResidualSSE(const Sim3 &referenceToFrame) {
  ...
  const __m128 depthVarFacs =
      _mm_set1_ps((float)settings.var_weight); // float depthVarFac =
                                               ...
                                               // var is over-confident. this is
                                               // a constant multiplier to
                                               // remedy that.... HACK
