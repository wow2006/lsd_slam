...
Undistorter *Undistorter::getUndistorterForFile(const char *configFilename) {
  ...
  // float ic[10];
  // if(std::sscanf(l1.c_str(), "%f %f %f %f %f %f %f %f %f %f", &ic[0], &ic[1],
...
UndistorterPTAM::UndistorterPTAM(const char *configFileName) {
  ...
  if (valid) {
    ...
    } else if (outputCalibration[0] == -2) // "full"
    {
      ...
      // find left-most and right-most radius
      float tl_radius =
          sqrt(left_radius * left_radius + top_radius * top_radius);
      float tr_radius =
      ...
      float hor =
          std::max(br_radius, tr_radius) + std::max(bl_radius, tl_radius);
      float vert =
      ...
      ofy =
          fy * ((vert) / (trans_vert)) * ((float)out_height / (float)in_height);
...
void UndistorterPTAM::undistort(const cv::Mat &image,
  ...
  if (image.rows != in_height || image.cols != in_width) {
    printf("UndistorterPTAM: input image size differs from expected input "
           "size! Not undistorting.\n");
    result.getMatRef() = image;
    return;
  }
  ...
  for (int idx = out_width * out_height - 1; idx >= 0; idx--) {
    ...
    if (xx < 0)
      ...
      data[idx] = xxyy * src[1 + in_width] + (yy - xxyy) * src[in_width] +
                  (xx - xxyy) * src[1] + (1 - xx - yy + xxyy) * src[0];
    }
  }
}

const cv::Mat &UndistorterPTAM::getK() const { return K_; }

const cv::Mat &UndistorterPTAM::getOriginalK() const { return originalK_; }

int UndistorterPTAM::getOutputWidth() const { return out_width; }

int UndistorterPTAM::getOutputHeight() const { return out_height; }

bool UndistorterPTAM::isValid() const { return valid; }
...
UndistorterOpenCV::UndistorterOpenCV(const char *configFileName) {
  ...
  K_ = cv::getOptimalNewCameraMatrix(
      camera_matrix, distortion_coefficients, cv::Size(img_width, img_height),
      outputCalibration, new_img_size, nullptr, false);
  ...
  //&inputCalibration[8], &inputCalibration[9]
  //  				) == 10 &&
  //			std::sscanf(l2.c_str(), "%d %d", &in_width, &in_height) ==
  //2)
  //	{
  //		printf("Input resolution: %d %d\n",in_width, in_height);
  ...
  //				inputCalibration[0], inputCalibration[1],
  //inputCalibration[2], inputCalibration[3], inputCalibration[4],
  //				inputCalibration[5], inputCalibration[6],
  //inputCalibration[7], inputCalibration[8], inputCalibration[9]);
  //	}
  //	else
  //	{
  ...
  //cv::Size(in_width, in_height), (outputCalibration == -2) ? 1 : 0,
  //cv::Size(out_width, out_height), nullptr, false);
  //
  //		cv::initUndistortRectifyMap(originalK_, distCoeffs, cv::Mat(),
  //K_,
  //				cv::Size(out_width, out_height), CV_16SC2, map1,
  //map2);
  //
  ...
  //		// TODO: PTAM code uses the following, should here also 0.5 be
  //subtracted?
  //// 		K_.at<double>(2, 0) = outputCalibration[2] * out_width - 0.5;
  //// 		K_.at<double>(2, 1) = outputCalibration[3] * out_height - 0.5;
  //
...
const cv::Mat &UndistorterOpenCV::getK() const { return K_; }

const cv::Mat &UndistorterOpenCV::getOriginalK() const { return originalK_; }

int UndistorterOpenCV::getOutputWidth() const { return out_width; }

int UndistorterOpenCV::getOutputHeight() const { return out_height; }

bool UndistorterOpenCV::isValid() const { return valid; }
}
